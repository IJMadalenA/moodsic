...
class RewardService:
    ...
    def _get_news_context(news_ids: list[int] | None) -> dict | None:
        ...
        if not news_items:
            return None
        avg_sentiment = sum(item.sentiment_score for item in news_items) / len(
            news_items
        )
        breaking_ratio = sum(1 for item in news_items if item.is_breaking) / len(
            news_items
        )
        return {
    ...
    def _get_user_history(user) -> dict:
        ...
        interactions = list(
            Interaction.objects.filter(user=user).select_related("track")
        )
        if not interactions:
            return {
                "skip_rate": 0.3,
