...
def sync_playlist_to_spotify(request, playlist_id: str):
    try:
        # 1. Obtenemos al usuario (martin) que es quien tiene los tokens
        # Usamos martin directamente para asegurar el testeo
        from django.contrib.auth import get_user_model
        User = get_user_model()
        target_user = User.objects.get(username='martin')

        # 2. Verificación de seguridad: ¿Tiene los tokens que vimos en el Admin?
        if not target_user.access_token:
            return Response(
                {
                    "success": False,
                    "error": "Usuario no está conectado a Spotify",
                    "message": "Los campos de token en el perfil de martin están vacíos.",
                },
                status=403,
            )

        # 3. Obtener la playlist local
        playlist = Playlist.objects.get(spotify_id=playlist_id)
        tracks = playlist.tracks.all()

        # 4. Inicializar servicio de Spotify con el usuario
        spotify_service = SpotifyMusicService(target_user)
        
        # 5. Crear playlist en Spotify
        spotify_playlist = spotify_service.create_playlist(
            name=playlist.name,
            description="Generated by Moodsic (Final Test)",
            public=playlist.is_public,
        )

        if not spotify_playlist:
            return Response({"success": False, "error": "Error al crear playlist"}, status=500)
        ...
        return Response({"error": "Playlist local no encontrada"}, status=404)
    except Exception as e:
