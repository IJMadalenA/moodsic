...
class SpotifyMusicService:
    ...
    def __init__(self, user):
        ...
        if self.client:
            try:
                ...
                print(
                    f"✅ [SERVICIO] Conexión establecida con ID de Spotify: {self.spotify_user_id}"
                )
    ...
    def get_spotify_client(self):
        ...
        try:
            ...
            if not token_obj:
                print(
                    f"❌ [AUTH] No existe SocialToken para el usuario {self.user.username}"
                )
                return None
            ...
            if not token_obj.expires_at or token_obj.expires_at <= timezone.now():
                print(
                    f"🔄 [AUTH] Token caducado. Iniciando refresco para {self.user.username}..."
                )
    ...
    def _refresh_token_process(self, token_obj):
        ...
        try:
            ...
            if new_info:
                ...
                # Sincronizar con el modelo User para que otros servicios (ML, etc.) tengan el token fresco
                ...
                print(
                    "✅ [REFRESH] Token actualizado en base de datos (SocialToken y User)."
                )
        except Exception as e:
    ...
    def create_playlist(self, name, description="Created by Moodsic", public=False):
        ...
        try:
            ...
            if e.http_status == 403:
                ...
                print(
                    f"2. Tu email ({self.user.email}) no está en 'User Management' del Dashboard de Spotify."
                )
            else:
