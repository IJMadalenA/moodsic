...
from apps.music.models import Track, Artist, Album, TrackAudioFeatures
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"Artistas cargados: {len(res['artists']['items'])}"))
        except Exception as e:
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"Álbumes cargados: {len(res['albums']['items'])}"))
        except Exception as e:
      ...
      # --- 3. CARGAR MUCHAS CANCIONES ---
       # --- 3. CARGAR MUCHAS CANCIONES (VERSIÓN MASIVA) ---
        ...
        busquedas = [
            'pop 2026', 'rock 80s', 'heavy metal', 'jazz classics', 'hip hop 2025',
            'electro house', 'reggaeton 2026', 'indie rock', 'blues', 'classical music',
            'k-pop', 'lofi beats', 'country music', 'disco hits', 'synthwave',
            'punk rock', 'salsa classics', 'techno underground', 'r&b soul', 'folk'
        ]
        ...
        for query in busquedas:
            ...
            try:
                ...
                if 'tracks' in results and 'items' in results['tracks']:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"Añadidas canciones de: {query}"))
                
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Fallo en {query}: {e}"))

        total = Track.objects.count()
        self.stdout.write(self.style.SUCCESS(f"¡LOGRADO! Ahora tienes {total} canciones en tu base de datos."))