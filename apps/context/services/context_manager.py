...
class ContextManager:
    ...
    def get_current_context(city, force_refresh=False):
        ...
        # 1. Gestionar Clima
        weather = WeatherContext.objects.filter(
            city=city, timestamp__gte=threshold
        ).first()
        if not weather or force_refresh:
            try:
                ...
                weather = WeatherContext.objects.filter(
                    city=city
                ).first()  # Fallback al último conocido
        ...
        return {"weather": weather, "news": news, "timestamp": now}
